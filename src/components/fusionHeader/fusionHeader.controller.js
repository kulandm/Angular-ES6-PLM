/**
 * @ngdoc object
 * @name Controllers.FusionHeaderController
 *
 * @description This controller is attached to Fusion header
 *		This should be removed after the unified component is provided
 *
 */
class FusionHeaderController {
	/**
	 * @ngdoc method
	 * @name Controllers.FusionHeaderController#constructor
	 * @methodOf Controllers.FusionHeaderController
	 * @description Controller constructor
	 */
	constructor($scope, $templateCache, $location, FlyoutService, $http, App) {
		this.$scope = $scope;
		...
		/**
		 * @ngdoc property
		 * @name Controllers.FusionHeaderController#helpFlyout
		 * @propertyOf Controllers.FusionHeaderController
		 * @description Holder for the help flyout instance
		 */
		this.helpFlyout = null;

		/**
		 * @ngdoc property
		 * @name Controllers.FusionHeaderController#hubName
		 * @propertyOf Controllers.FusionHeaderController
		 * @description It's the hubname
		 */
		this.hubName;

		this.$http.get(root.hubs.link).then((response) => {
			if (response.items && response.items[0].name) {
				this.hubName = response.items[0].name;
			}
		});
		
		this.addSvgIconsToCache();

		/* Inject query param from url bar for search box */
	...
	addSvgIconsToCache() {
		...
		// Source https://git.autodesk.com/A360/Galileo-Web/blob/master/app/styles/icons/walkthrough.svg
		this.$templateCache.put('fusion-header-walkthrough-icon-template',
			'<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMid" width="31" height="31" viewBox="0 0 31 31"><defs><style>.walk-1, .walk-2, .walk-3 {\r' +
			'\n' +
			'        fill: #24b5ed;\r' +
			'\n' +
			'      }\r' +
	...
	/**
	 * @ngdoc method
	 * @name Controllers.FusionHeaderController#openHelpFlyout
	 * @methodOf Controllers.FusionHeaderController
	 * @description Opens the help flyout
	 */
	openHelpFlyout(event) {
...
export default FusionHeaderController;

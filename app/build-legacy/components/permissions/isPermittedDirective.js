 ...
 * </doc:example>
 */
angular.module('plm360.permissions').directive('isPermitted', ['$stateParams', '$parse', 'PermissionService', 'UrnParser', function ($stateParams, $parse, PermissionService, UrnParser) {
	return {
		restrict: 'A',
    ...
    */
			var updateVisibility = function updateVisibility(isVisible) {
				if (!isVisible) {
					ele.remove();
				}
			};
    ...
    */
			...
			if (angular.isDefined(attrs.workspaceId) || angular.isDefined($stateParams.workspaceId)) {
				PermissionService.hasPermissions([parseInt(attrs.isPermitted)], attrs.workspaceId || $stateParams.workspaceId).then(updateVisibility);

				attrs.$observe('workspaceId', function (value) {
					PermissionService.hasPermissions([parseInt(attrs.isPermitted)], value).then(updateVisibility);
				});
			}

			if (ele.length && angular.isDefined($stateParams.itemId)) {
				PermissionService.checkPermissionByItemId(UrnParser.encode($stateParams.itemId), parseInt(attrs.isPermitted), scope.applyLock, scope.applyRevisionOverrideLock).then(updateVisibility);
			}
		}
	};
}]);
//# sourceMappingURL=isPermittedDirective.js.map

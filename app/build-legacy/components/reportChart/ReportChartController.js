 ...
 *
 */
angular.module('plm360.reportChart').controller('ReportChartController', ['$scope', '$rootScope', '$q', '$filter', 'LocalizationService', '_', '$modal', 'ReportChartConstants', function ($scope, $rootScope, $q, $filter, LocalizationService, _, $modal, ReportChartConstants) {
  ...
  */
					...
					switch ($scope.chartType) {
						...
						case 'STACKEDAREA':
							...
							if (data.reportDefinition.reportChart.xAxisSeriesField) {
								...
								$scope.chartData = that.createSeries(that.createSeriesMap(seriesId, data.reportResult.row), $scope.categories, xAxisId, yAxisId, $scope.chartType === 'MSCOLUMN' || $scope.chartType === 'STACKEDCOLUMN' ? null : 0);
							} else {
								...
								$scope.categories = that.createCategoryList(xAxisId, data.reportResult.row);
								$scope.chartData = that.createSeriesForAggregates(data.reportResult.row, seriesId, that.createFieldDescriptor(data.reportDefinition.reportChart.yAxisSeriesField), yAxisId, that.createFieldDescriptor(data.reportDefinition.reportChart.yAxisField));
							}
							break;
     ...
     */
				...
				if ($scope.chartType === 'LINE' || $scope.chartType === 'MSLINE' || $scope.chartType === 'AREA' || $scope.chartType === 'MSAREA' || $scope.chartType === 'STACKEDAREA') {
					useTicks = true;
				}
...
}]).controller('ReportChartModal', ['$scope', '$modalInstance', '$timeout', 'reportObj', 'title', function ($scope, $modalInstance, $timeout, reportObj, title) {
...
//# sourceMappingURL=ReportChartController.js.map

 ...
 */
/* global plm360 */
var CommentsService = plm360.factory('CommentsService', ['$http', '$q', 'PLM360.Token.service', '_', function ($http, $q, PLMTokenService, _) {
  ...
  */
   ...
   *
   */
		postReply: function postReply(comment, tags) {
			var deferred = $q.defer();
			PLMTokenService.get().then(function (token) {
				$http({
					method: 'POST',
   ...
   *
   */
		postComment: function postComment(urn, commentText, tags) {
			var deferred = $q.defer();
			PLMTokenService.get().then(function (token) {
				$http({
					method: 'POST',

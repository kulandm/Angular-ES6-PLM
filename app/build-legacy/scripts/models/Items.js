...
Items.prototype = {
  ...
  */
	fetch: function fetch(link, params, workspaceId) {
		var that = this;
  ...
  */
	getItemLink: function getItemLink(workspaceId, dmsId) {
		var ret = '';
  ...
  */
	setItemObj: function setItemObj(dmsId, obj) {
		angular.forEach(this.json.items, function (item) {
			if (item.dmsId === dmsId) {
				item.obj = obj;
			}
...
angular.module('plm360.models').factory('Items', ['RESTWrapperService', '$q', 'EventService', 'UrnParser', function (RESTWrapperService, $q, EventService, UrnParser) {
	var models = {};

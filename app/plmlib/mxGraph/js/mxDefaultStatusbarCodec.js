/**
 * $Id: mxDefaultToolbarCodec.js,v 1.21 2010-09-30 09:32:38 gaudenz Exp $
...
mxCodecRegistry.register(function()
{
	 ...
	 */
	var codec = new mxObjectCodec(new mxDefaultStatusbar());
			...
			while (node != null)
			{
				if (node.nodeType == mxConstants.NODETYPE_ELEMENT)
				{
					if (!this.processInclude(dec, node, into))
					{
						...
						{
							elt = into.addSeparator();
						}
						else if (node.nodeName == 'br')
						{
							elt = into.toolbar.addBreak();
						}
						else if (node.nodeName == 'hr')
						{
							elt = into.toolbar.addLine();
						}
						else if (node.nodeName == 'add')
						{
							...
							var showText = node.getAttribute('showText');
							var text = mxUtils.getTextContent(node);

							if (action != null)
							{
								if (action != 'zoomCustom') {
									elt = into.addItem(as, icon, action, pressedIcon, null, null, showText);
								} else {
									elt = into.addZoomCustom();
								}
							}
							else if (mode != null)
							{
